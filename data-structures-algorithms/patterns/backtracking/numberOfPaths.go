...
// commented code is for the inefficient version that stores all paths as strings
func NumOfPathsToDest(n int) int {
	...
	/*
			...
			return len(res)
	*/
	return visit(0, 0, n, 0, 0)
}

// func visit(i, j, n int, path string, res []string) []string {
func visit(i, j, n, pathLen, count int) int {
	if i >= n || j >= n {
		//return res
		return count
	}
	...
	if i+1 < n {
		...
		count = visit(i+1, j, n, pathLen+1, count)
	}
	if j+1 < n && j+1 <= i {
		// res = visit(i, j+1, n, path+"N", res)
		count = visit(i, j+1, n, pathLen+1, count)
	}
	//return res
	return count
}
